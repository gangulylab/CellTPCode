...
% Pool cell depth, index, event counts, and modulation flag across animals for
% each session, then plot the post/pre event-count ratio against depth.
    um_x   = 950/1280;
    um_y   = 600/800;
    ...
    % Event-ratio vs depth for all sessions (skips if event data missing)
    try
        figure
    ...
    % Event-ratio vs depth for session 2 only
    figure
    for s = 2
        tmp_depth = Mdata_in{s}.depth;
