% Modulated vs non-modulated cell counts binned by cortical depth, shown as
% stacked percentage bars per stage (transplant) and for the thy1 control.

Mdata_comb      = comb4depth_v2(Mdata,3);
Mdata_thy1_comb = comb4depth_v2(Mdata_thy1,1);
bin_range = 0:200:1000;   % depth bin edges (microns)

% Transplant: bin modulated / non-modulated cells by depth
for s = 1:size(Mdata_comb,2)
    tmp_depth = Mdata_comb{s}.depth-100;   % 100 um subtraction for depth correction
